...
# --- RoPE + entropy-invariant attention --------------------------------------


def _rope_dit(**overrides):
...
def test_rotary_embedding_is_built_and_reaches_the_blocks():
    dit = _rope_dit()
    assert dit.rotary_embed is not None
    freqs, _ = dit.rotary_embed.forward_from_seq_len(20)
    assert freqs.shape[-2] == 20  # [1, n, d]: one set of positions broadcast over the batch
...
def test_rope_breaks_the_permutation_equivariance_of_full_attention():
    """Full attention on its own is permutation-equivariant, so it carries no position.

    Rotating the queries and keys by their absolute index is what breaks that. (In the
    assembled DiT `ConvPositionEmbedding` also supplies local relative position over a
    61-frame window, so this isolates the rope's own contribution.)
    """
    ...
    plain = F.scaled_dot_product_attention(q, k, v, scale=1.0)
    plain_perm = F.scaled_dot_product_attention(q[:, :, perm], k[:, :, perm], v[:, :, perm], scale=1.0)
    assert torch.allclose(plain[:, :, perm], plain_perm, atol=1e-5)

    freqs, _ = RotaryEmbedding(d).forward_from_seq_len(n)
    qr, kr = apply_rotary_pos_emb(q, freqs, 1.0), apply_rotary_pos_emb(k, freqs, 1.0)
    roped = F.scaled_dot_product_attention(qr, kr, v, scale=1.0)
    qp, kp = apply_rotary_pos_emb(q[:, :, perm], freqs, 1.0), apply_rotary_pos_emb(k[:, :, perm], freqs, 1.0)
    roped_perm = F.scaled_dot_product_attention(qp, kp, v[:, :, perm], scale=1.0)
    assert not torch.allclose(roped[:, :, perm], roped_perm, atol=1e-4)
...
def test_logn_scaling_sharpens_but_never_flattens():
    ...
    assert proc._logit_scale(400) == pytest.approx(math.log(400) / math.log(100))
...
def test_attention_output_projection_is_not_widened():
    """Full attention emits head_dim per head, unlike the two-direction causal variant."""
    attn = _rope_dit().transformer_blocks[0].attn
...
# --- overlapping patches + windowed overlap-add ------------------------------


def _patch_dit(overlap, **overrides):
    from wavtts.model.backbones.dit import DiT

    kwargs = dict(dim=64, depth=2, heads=2, dim_head=32, ff_mult=2, wav_frame_len=160, patch_overlap=overlap)
    kwargs.update(overrides)
    return DiT(**kwargs)


@pytest.mark.parametrize("overlap", [0, 40, 80])
@pytest.mark.parametrize("num_samples", [160, 1600, 3000, 4321])
def test_patch_roundtrip_is_exact(overlap, num_samples):
    """Analysis then synthesis must be the identity: every patch carries the true samples,
    so any weighted mean of them is the true sample again — whatever the window."""
    torch.manual_seed(0)
    dit = _patch_dit(overlap)
    x = torch.randn(2, num_samples)

    tokens, _, _ = dit._wav_to_tokens(x)
    assert tokens.shape == (2, math.ceil(num_samples / 160), 160 + 2 * overlap)

    y = dit._tokens_to_wav(tokens, num_samples)
    assert y.shape == x.shape
    assert torch.allclose(x, y, atol=1e-5)


def test_overlap_add_holds_amplitude_at_the_edges():
    """The outermost samples are reached by one patch only, at the taper of its window.

    A plain COLA sum would fade the clip in and out there; dividing by the true window
    envelope keeps them at full amplitude.
    """
    dit = _patch_dit(80)
    x = torch.ones(1, 800)
    y = dit._tokens_to_wav(dit._wav_to_tokens(x)[0], 800)

    assert torch.allclose(y, torch.ones_like(y), atol=1e-6)
    # and the envelope really is non-flat, so the test is not vacuous
    env = dit.synthesis_window[None, :, None].expand(1, dit.patch_len, 5)
    env = torch.nn.functional.fold(env, (1, 4 * 160 + 320), (1, dit.patch_len), stride=(1, 160)).flatten()
    assert env[0] < 0.5 * env[len(env) // 2]


def test_overlap_leaves_the_token_grid_alone():
    """Tokens, mask and lengths stay hop-aligned, so overlap changes width, not count."""
    torch.manual_seed(0)
    x = torch.randn(2, 1000)
    mask = torch.zeros(2, 1000, dtype=torch.bool)
    mask[0, :1000] = True
    mask[1, :500] = True
    lens = torch.tensor([1000, 500])

    plain = _patch_dit(0)._wav_to_tokens(x, mask=mask, lens=lens)
    over = _patch_dit(80)._wav_to_tokens(x, mask=mask, lens=lens)

    assert plain[0].shape[1] == over[0].shape[1] == 7  # ceil(1000/160)
    assert torch.equal(plain[1], over[1])
    assert torch.equal(plain[2], over[2])


def test_patch_overlap_widens_the_io_projections():
    dit = _patch_dit(80, use_audio_proj=True, audio_proj_dim=64, audio_proj_hidden=64)
    assert dit.patch_len == 320
    assert dit.input_embed.x_proj[0].in_features == 320
    assert dit.proj_out.out_features == 320


def test_patch_overlap_rejects_more_than_half_a_hop():
...
def test_set_wav_frame_len_accounts_for_the_overlap():
    dit = _patch_dit(80)
    dit.set_wav_frame_len(160)  # the hop, not the patch: must be accepted
    with pytest.raises(ValueError, match="must equal"):
        dit.set_wav_frame_len(200)
