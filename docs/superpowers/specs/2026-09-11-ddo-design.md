...
（`beta_mel` 極小）低估它一個數量級。

R2Mel2 以 `beta_mel 4` 重跑（mel logit ≈ 1）：@50 3.626 / 0.068、@100 3.556 / 0.047、
@150 3.468 / 0.035——與 β_mel 16 在雜訊內相同。**β_mel 在 4–16 之間不改變 run 的走向**，
mel 通道給的是一條沿輪次展開的 silence / UTMOS 取捨曲線：50 步是起點的 UTMOS 加略少的靜音，
100 步是 base 的 silence（0.05）配 +0.53，150 步是真實語料的 silence（0.035）配 +0.44。
UTMOS 偏好乾淨的靜音，所以哪個點「聽起來」最好是聽測的問題，指標決定不了。
候選：`R2Mel2/model_50.pt`（3.63 / 0.068）與 `R2Mel2/model_100.pt`（3.56 / 0.047）。
