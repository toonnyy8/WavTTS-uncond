...
`max(1, log(n) / log(logn_ref_len))` 讓熵維持穩定。

與論文無關，是獨立開關，也**訓練時就套用**。理由是這個模型跟 LLM 的處境不同：LLM 訓練長度
固定，只能在推論期硬套一個沒學過的係數；而我們的 clip 長度天然橫跨 40–2979 frames，
而且 batch 依長度排序，所以模型有機會在跨一個數量級的 `n` 上**學到** `n ↔ scale` 的對應，
外推出去的那個值才有意義。

**下限截斷與參考長度（2026-08-20 修正）**。最初設 `logn_ref_len = 3000`（訓練最長 clip）
且不截斷，實測是錯的——整個訓練分佈的係數都 < 1：

| | frames | ref=3000 無截斷 | ref=500 截斷於 1 |
|---|---|---|---|
| min | 40 | 0.461 | 1.000 |
| p05 | 105 | 0.581 | 1.000 |
| median | 455 | 0.764 | 1.000 |
| p95 | 1517 | 0.915 | 1.179 |
| max | 2979 | 0.999 | 1.287 |
| 生成 5s | 500 | 0.776 | 1.000 |
| 生成 30s | 3000 | 1.000 | 1.288 |
| 生成 120s | 12000 | 1.173 | 1.511 |

也就是說這個機制從頭到尾只在**壓平**注意力，從來沒有機會做它真正該做的事（為超長序列銳化）。
最短的 clip 被壓到 0.461，等於把 40 個 key 的 softmax 推向均勻分佈；連生成 5s 時也壓了 22%。
Su 的原始 formulation 與 Qwen 的實作都夾 `max(1, log_m(n))`，正是因為壓平不是目的。

改為 `logn_ref_len = 500`（5 秒，接近 median，也等於 checkpoint 取樣長度）並截斷於 1：
5 秒以下維持 1.0、30 秒 1.29、120 秒 1.51。訓練分佈的上半段（500–2979 frames，6× 範圍）
仍然是「學進去」的，外推到 12000 才有依據。
