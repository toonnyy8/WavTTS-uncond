...
| `ddo.beta_mel` | 0.0 | > 0 opens a second discriminator channel in the multi-scale log-mel domain: `Δ_mel = −(mel_θ − mel_ref)` per row (raw log-mel L1 units), and the logit becomes `β·Δ_v + β_mel·Δ_mel` — one sigmoid, so both channels saturate together. Calibrate `β_mel` so `β_mel·std(Δ_mel) ≈ β·std(Δ_v)` at the first sign of movement; `WavTTS_ddo_r2_mel.yaml` carries the calibrated value. Not an ELBO term: Theorem 3.1/3.3 do not cover it |
